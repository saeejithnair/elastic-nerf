...
import wandb
from elastic_nerf.nerfacc.trainers.vanilla_nerf import (
    VanillaNeRFTrainer,
    VanillaNeRFTrainerConfig,
)


def main(config: VanillaNeRFTrainerConfig):
    trainer: VanillaNeRFTrainer = config.setup()
    ...
    main(
        tyro.cli(
            tyro.conf.SuppressFixed[  # Don't show unparseable (fixed) arguments in helptext.
                tyro.conf.FlagConversionOff[VanillaNeRFTrainerConfig]
            ],
