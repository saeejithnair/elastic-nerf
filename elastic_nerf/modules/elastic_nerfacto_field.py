...
from elastic_nerf.modules.elastic_mlp import ElasticMLP


class ElasticNerfactoField(Field):
    ...
    def __init__(
        ...
        self.elastic_mlp = ElasticMLP(
            ...
            activation=nn.ReLU(),
            out_activation=None,
            use_granular_norm=use_granular_norm,
        )
    ...
    def get_density(
        self, ray_samples: RaySamples, active_neurons: Optional[int] = None
    ) -> Tuple[Tensor, Tensor]:
        ...
        # Call the hash encoding and ElasticMLP separately with the
        # positions and active_neurons.
        with autocast():
            encoded_positions = self.mlp_base_grid(positions_flat)
            h = self.elastic_mlp(encoded_positions, active_neurons).view(
        ...
        if compute_normals:
            with torch.enable_grad():
                density, density_embedding = self.get_density(
                    ray_samples, active_neurons
                )
        else:
            density, density_embedding = self.get_density(ray_samples, active_neurons)
